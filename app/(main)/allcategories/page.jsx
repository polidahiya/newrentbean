    ...
    <div className="px-2 md:px-10 py-5">
      {Object.entries(categorylist).map(([key, item], i) => (
        <div key={i}>
          ...
          >
            <p className="font-recline text-xl tracking-wide">{key?.replace(/-/g," ")}</p>
            <p className="font-recline text-xl tracking-wide text-theme">View All</p>
          </Link>
            ...
            {item?.subcat.map((subcat, j) => {
              return (
                ...
                >
                  ...
                  ></Image>
                  <p className="text-center text-sm mt-1">{subcat?.name?.replace(/-/g," ")}</p>
                </Link>
              );
            })}
          </div>
